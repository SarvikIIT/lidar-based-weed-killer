...
%
%   Mathematical models:
%       Collimated divergence:  theta = lambda / (pi * w0)
%       Beam waist:             w0 = f * NA  (approximation)
...
%% -----------------------------------------------------------------------
%  Beam Collimation Model
%  -----------------------------------------------------------------------
%  w0 ≈ f × NA  (simple thin-lens approximation for the laser diode)
w0 = tx.lens_focal * tx.lens_NA;               % beam waist [m]

%  Collimated divergence (half-angle, diffraction-limited):
%     theta_coll = lambda / (pi * w0)
theta_coll = tx.wavelength / (pi * w0);        % [rad]

%  Rayleigh range
z_R = pi * w0^2 / tx.wavelength;               % [m]

beam.w0             = w0;
beam.divergence_coll = theta_coll;              % collimated half-angle divergence
beam.divergence_raw  = tx.beam_divergence;      % raw diode divergence
