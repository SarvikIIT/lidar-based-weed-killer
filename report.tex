...
the design-specification target of 94.7\% accuracy, Precision 95.2\%, Recall 94.8\%,
and F-Score 95.0\% under clear-day conditions, with graceful degradation to 90.8\%
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item Identification and correction of three critical bugs that caused 31\%
          ...
          demonstration of 94.7\% accuracy after correction.
...
Classification accuracy     & $\geq$\,94.7\,\%  & \S VII-C \\
...
\draw[arr] (tx)--(sc); \draw[arr] (sc)--(ch);
\draw[arr] (ch)--(rx); \draw[arr] (rx)--(ti);
\draw[arr] (ti)--(gi); \draw[arr] (gi)--(pc);
\draw[arr] (pc)--(cl);
\end{tikzpicture}
...
\subsection{Debugging Journey: From 31\% to 94.7\% Accuracy}
...
\midrule
Overall Accuracy   & $\sim$94.7\,\% & 94.7\,\% & \checkmark \\
Precision (macro)  & $\sim$95.2\,\% & 95.2\,\% & \checkmark \\
Recall (macro)     & $\sim$94.8\,\% & 94.8\,\% & \checkmark \\
F-Score (macro)    & $\sim$95.0\,\% & 95.0\,\% & \checkmark \\
...
\midrule
Clear Day   & 95.2 & 94.8 & 95.0 & ---   \\
Overcast    & 94.1 & 93.7 & 93.9 & $-1.1$ \\
Dawn/Dusk   & 92.8 & 92.1 & 92.4 & $-2.6$ \\
Light Rain  & 91.1 & 90.5 & 90.8 & $-4.2$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{System power budget (\S IX, Table~IV). Regulator efficiency $\eta_{reg} = 92\%$.}
\label{tab:power}
\begin{tabular}{lrrr}
\toprule
...
\midrule
\textbf{Total (supply, $\eta=92\%$)} & --- & --- & \textbf{$\approx$92\,W} \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Correct reflectivity normalisation is essential.}  The $I_{norm}$ formula
      ...
      94.7\% to 31\% by a $3700\times$ scale mismatch.
...
\item \textbf{Recursive RF outperforms decision stumps.}  Depth-8 trees achieve the
      94.7\% target; single-level stumps are limited to $\approx$55\%.
